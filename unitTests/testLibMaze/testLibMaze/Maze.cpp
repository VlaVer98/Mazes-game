...
void Maze::create() {
	...
	for (int i = 0; i < height; i++) // «аполн€ем поле
	...
	while (a < 10000) {
		...
		while (1) { // Ѕесконечный цикл, который прерываетс€ только тупиком
			...
			switch (c) {
			case 0: if (y != 1)
				if (field[y - 2][x] == wall) { // ¬верх
					field[y - 1][x] = blank;
					field[y - 2][x] = blank;
					y -= 2;
				}
			case 1: if (y != height - 2)
				if (field[y + 2][x] == wall) { // ¬низ
					field[y + 1][x] = blank;
					field[y + 2][x] = blank;
					y += 2;
				}
			case 2: if (x != 1)
				if (field[y][x - 2] == wall) { // Ќалево
					field[y][x - 1] = blank;
					field[y][x - 2] = blank;
					x -= 2;
				}
			case 3: if (x != width - 2)
				if (field[y][x + 2] == wall) { // Ќаправо
					field[y][x + 1] = blank;
					field[y][x + 2] = blank;
					x += 2;
				}
...
void Maze::createInOut() {
	...
	//unit test 2
	cout << "test 2(метод createInOut):" << endl;
	for (int i = 0; i < height; i++) {
		for (int j = 0; j < width; j++) {
			cout << field[i][j] << " ";
		}
...
bool Maze::findExit(int ax, int ay, int bx, int by) {
	...
	if (fieldCopy[ay][ax] == wall || fieldCopy[by][bx] == wall) return false; // €чейка (ax, ay) или (bx, by) - стена

	// распространение волны
	d = 0;
	fieldCopy[ay][ax] = 0; // стартова€ €чейка помечена 0
	do {
		stop = true;  // предполагаем, что все свободные клетки уже помечены
		for (y = 0; y < height; ++y)
			for (x = 0; x < width; ++x)
				if (fieldCopy[y][x] == d)  // €чейка (x, y) помечена числом d
				{
					for (k = 0; k < 4; ++k) // проходим по всем непомеченным сосед€м
					{
							...
							fieldCopy[iy][ix] == blank)
						{
							...
							fieldCopy[iy][ix] = d + 1; // распростран€ем волну
						}
					}
				}
		d++;
...
void Maze::changeTrueExit() {
	...
	//unit test 3
	cout << "test 3(метод changeTrueExit):" << endl;
...
void Maze::drawTruePath() {
	...
	//unit test 4
	cout << "test 4(метод drawTruePath):" << endl;
	for (int i = 0; i < height; i++) {
		for (int j = 0; j < width; j++) {
			cout << field[i][j] << "\t";
		}
